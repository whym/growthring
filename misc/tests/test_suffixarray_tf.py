...
class TestSuffixArrayTF(unittest.TestCase):
    ...
    def test_lcp(self):
        s = 'ABRACADABRA#'
        self.assertEqual([-1, 0, 1, 4, 1, 1, 0, 3, 0, 0, 0, 2],
                         st.longest_common_prefixes(s, st.suffixes(s)))
    ...
    def test_all_subspans0(self):
        ...
        self.assertEqual(sorted(list(p1)),
                         sorted([(x[0]+1, x[1]+1) for x in p2]))

    def test_pincer(self):
        s = 'ABCDE'
        q = 'BCD'
        m = 'C'
        self.assertEqual(['BC', 'C', 'BCD', 'CD'],
                         list(st.pincer(s, get_pos(s, q), [get_pos(s, m)])))

    def test_tf(self):
        s = 'mississippi' + st.DELIMIT
        ec = list(st.equivalence_classes(s))
        self.assertEqual(['issi', 'i', 'mississippi', 'p', 's'],
                         [s[x.pos[0]:x.pos[1]] for x in ec if x])
