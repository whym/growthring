...
/**
  * @author Yusuke Matsubara <whym@whym.org>
  */
class TestCovering extends AnyFunSuite {
  ...
  test("covering greedy conserv") {
    assertResult(Set(0, 1, 5, 6, 7, 8, 10, 11)) {
      Covering.greedyConservative(
        "abcdefghi abcdef".toCharArray,
  ...
  test("covering dp") {
    assertResult(List(0, 1, 2, 3, 10, 11, 12, 13, 14, 15, 16, 17, 18)) {
      Covering.dp(
        "abcdefghijklmnopqrs".toCharArray,
        List((0, 3), (3, 10), (10, 18))).toList.sorted
    }
  }
  test("covering dp 2") {
  ...
  test("exhaustive1") {
    assertResult(List(1, 2, 3, 6, 7, 8)) {
      Covering.exhaustive(
        "abcdefghijklmnopqrs".toCharArray,
        List((1, 3), (4, 6), (6, 8))).toList.sorted
    }
  }
  ...
  test("dp gap=1") {
    assertResult(List(1, 2, 3, 6)) {
      Covering.dp(
        "abcdefghijklmnopqrs".toCharArray,
        List((1, 3), (3, 4), (6, 6)), 1).toList.sorted
    }
  }
  ...
  test("dp gap=0") {
    assertResult(List(1, 2, 3, 4, 6)) {
      Covering.dp(
        "abcdefghijklmnopqrs".toCharArray,
        List((1, 2), (3, 4), (6, 6)), 0).toList.sorted
    }
  }
