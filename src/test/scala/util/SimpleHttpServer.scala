...
object SimpleHttpServer {
  class MyHandler(path: String, body: String, ctype: String)
      extends HttpHandler {
    @throws[IOException]
  ...
  def create(
    ...
    for (path, body) <- map do {
      val ctype =
        if body.size > 0 && body(0) == '<' then { "text/xml" }
        else if body.size > 0 && body(0) == '{' then { "application/json" }
        else { "text/plain" }
      val slash_path =
        if path(0) == '/' then { path }
        else { "/" + path }
  ...
  def main(args: Array[String]): Unit = {
    import scala.util.Properties.{propOrElse as p}
    val addr = findFreeAddress()
    val s = create(
      p("host", "localhost"),
      p("port", addr.getPort.toString).toInt,
      Range(1, 10).map(x =>
        (p("path" + x, x.toString), p("body" + x, "OK"))
      ).toMap
    )
