      ...
      s =>
        {
          ...
          for
            ...
            r = rp(n)
          do {
            for i <- Range.inclusive(r._1, r._2) do {
              ar(i) = c
            }
            c += 1
          }
          (-1 +: -1 +: ar).lazyZip(-1 +: ar :+ -1).lazyZip(ar :+ -1 :+ -1).map {
            ...
            case (-1, _, -1) => Single()
            case (x, y, z) => (
              if x == y && y == z then { Inside() }
              else if x == y then { End() }
