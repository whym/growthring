...
/**
  * A set of functions to convert a set of repeats into text, annotated texts,
  * etc
  *
  * @author
  *   Yusuke Matsubara <whym@whym.org>
...
case class Repeat(length: Int, positions: Set[Int])
  ...
  def asSlicesOf[T](body: Seq[T]) = new Iterable[Seq[T]] {
    def iterator =
      spans.map(_ match { case (i, j) => body.slice(i, j) }).iterator
  }
}
