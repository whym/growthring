...
object Main extends LazyLogging {
  ...
  def findCovereds(
    ...
    while lit.hasNext do {
      ...
      while sit.hasNext && sit.head._1 < lit.head._1 do {
        sit.next()
      }
      val buf = new mutable.ArrayBuffer[(Int, Int)]
      while sit.hasNext && inside(sit.head, lit.head) do {
        buf.append(sit.head)
        sit.next()
      }
      if buf.size >= supports then {
  ...
  def anonymize(
    ...
    logger.debug(
      f"${str.length}%d characters, frequency at least ${freq}%d, each unprotected span at least ${min_len}%d in length."
    )
    val covered = for ((s, e) <- rmethod(str, freq); if e - s + 1 >= min_len)
      yield (s, e)

    val start =
      if _start >= 0 then { _start }
      else { 0 }
    val end =
      if _end > 0 then { _end }
      else { str.length }
    val coveredFiltered = covered.filter(x =>
      (start <= x._1 && x._1 < end) || (start <= x._2 && x._2 < end)
    )
    logger.debug(
      f"${coveredFiltered.size}%d / ${covered.size}%d repeats from ${start}%d to ${end}%d."
    )
    ...
    for x <- (new Regex(unhide_pattern) findAllMatchIn str) do {
      for i <- Range(x.start, x.end) do {
        unhides(i) = true
      }
    }
  ...
  def findBoundariesAsArray(str: String, pattern: Regex): IndexedSeq[Int] = {
    ...
    for x <- (pattern findAllMatchIn str) do {
    ...
    scala.collection.immutable.ArraySeq.unsafeWrapArray(bd)
  }

  def findBoundaries(str: String, pattern: Regex): Int => Int = {
    val bd = findBoundariesAsArray(str, pattern)

    if bd.length > 0 && bd(0) != str.length then { bd }
    else { _ => str.length + 1 }
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    val modes: Map[String, ExecMode] = Map(
      "multiple-anonym" -> ExecMode(
        ...
        { strings =>
          ...
          for
            config <- XhtmlParser(
              io.Source.fromFile(config.getString("configFile"))
            ) \\ "config"
          do {
      ...
      "anonym" -> ExecMode(
        ...
        { strings =>
          ...
          val rmethod: (String, Int) => Seq[(Int, Int)] =
            config.getString("repeatsMethod") match {
              case "naive" => NaiveExtremalSubstrings.maxRepeats
              case "word"  => new WordRepeats().repeats
              case "ngram" =>
                new NgramRepeats(config.getInt("ngramSize")).repeats(
                  _,
                  _,
                  config.getInt("minLen").toInt
                )
              case m => { (s: String, r: Int) =>
                new ExtremalSubstrings(SuffixArrays.build(s, m)).maxRepeats(r)
              }
            }

          val cmethod: (Array[Char], Seq[(Int, Int)], Int) => Set[Int] =
            config.getString("coveringMethod") match {
              ...
              case x => {
                logger.debug(
                  s"covering method '${x}' not found; using default covering algorithm"
                )
      ...
      "msa" -> ExecMode(
        Seq("find multiple sequence alignment"),
        { strings =>
          val msa = new MultipleSequenceAlignment[Char](strings.map(x =>
            ("^" + x + "$").toCharArray.toIndexedSeq
          ))
          val dag = msa.align().compact((x, y) => x.concat(y))
          def nodeformat(
              i: Int,
              x: MultipleSequenceAlignment.Node[Char]
          ): String = {
            def escape(x: String) =
              x.replace("\\", "\\\\").replace("\"", "\\\"")
            "  N_%d[label=\"%s\",fontsize=%f];".format(
              i,
              escape(x.label.map(_.toString).mkString),
              10 + scala.math.log(x.freq) * 4
            )
          }
          for line <- dag.dot(nodeformat) do {
            println(line)
          }
        }
      ),
      "dfreq" -> ExecMode(
        Seq("calculate document frequencies"),
        { strings =>
          val str = strings.mkString("\n")
          val bd =
            findBoundariesAsArray(str, new Regex(config.getString("boundary")))
          ...
          def tfindnaive(s: String, d: Int) =
            NES.find(str, s).filter(bd(_) == d)
          ...
          for pos <- rps do {
            ...
            for
              n <-
                Range.inclusive(config.getInt("minLen"), slice.length).reverse
            do {
              for i <- Range.inclusive(0, slice.length - n) do {
                val s = slice.slice(i, i + n)
                val atf = sa.find(s).size
                val tf = tfind(s, doc).size
                val df = dfind(s).size
                println("%d\t%d\t%d\t%d\t%d\t%d\t%.2f\t%d\t%s".format(
                  pos._1,
                  pos._1 + i,
                  pos._2 + i + n,
                  atf,
                  tf,
                  df,
                  tf * scala.math.log(1.0 + 1.0 * ndocs / df),
                  s.size,
                  s
                ))
      ...
      "nested" -> ExecMode(
        ...
        { strings =>
          ...
          val es = new ExtremalSubstrings(SuffixArrays.build(
            str,
          ...
          if mr > mu then {
            logger.debug("warning: repeats (%s) should not be greater than nested repeats (%s)".format(
              mr,
              mu
            ))
          }
          val rps = es.maxRepeats(mr, bd)
          val uqs = if config.getBoolean("nested.useUniques") then {
            es.minUniques(mu, bd)
          } else {
            es.maxRepeats(mu, bd)
          }

          for (rp, uq) <- findCovereds(rps, uqs, supports) do {
    ...
    modes.get(modeName) match {
      case Some(exec) =>
        import scala.io
        val strings = if args.length > 0 then {
      ...
      case None =>
        ...
        System.err.println(
          s"mode '${modeName}' not found; choose one from ${modes.keys.mkString(", ")} and set it to -Dorg.whym.growthring.mode=:"
        )
        modes.foreach {
          case (name, e) => {
